...
class SignInViewController: UIViewController, GIDSignInUIDelegate, LoginButtonDelegate {
    ...
    fileprivate func saveUserIntoFirebaseDatabase() {
        ...
        dateToday.setLocalizedDateFormatFromTemplate("EEEE, yyyy")
        ...
        self.ref.child("users").child(uid).observeSingleEvent(of: DataEventType.value, with: { (snapshot) in
            ...
            if snapshot.exists() == false {
                ...
                self.ref.child("users").child(self.uid).setValue(["uid": self.uid, "name": self.name, "email": self.email, "profileImageUrl": self.profileImageUrl, "provider": self.provider, "dateDeCreation": self.dateDeCreation])
            }
        }, withCancel: nil)
