...
class AccueilViewController: UIViewController, UIScrollViewDelegate {
    ...
    let transport: BtnPleinLarge = {
        ...
        cours.addRightImage(image: icon!, offset: 30)
        cours.backgroundColor = #colorLiteral(red: 0.3460007906, green: 0.3354875743, blue: 0.8400039077, alpha: 1)
    ...
    func setupLayout() {
        ...
        mesCours.heightAnchor.constraint(equalToConstant: 100).isActive = true
        ...
        DemandeDeDocumentsScolarite.topAnchor.constraint(equalTo: Blog.bottomAnchor, constant: 20).isActive = true
        DemandeDeDocumentsScolarite.widthAnchor.constraint(equalToConstant: view.frame.width - 40).isActive = true
        DemandeDeDocumentsScolarite.heightAnchor.constraint(equalToConstant: 100).isActive = true
        ...
        InfoUtile.heightAnchor.constraint(equalToConstant: 100).isActive = true
        ...
        transport.heightAnchor.constraint(equalToConstant: 100).isActive = true
        ...
        biblio.bottomAnchor.constraint(equalTo: contentView.bottomAnchor, constant: -60).isActive = true
    }
    ...
    @objc func buttonToMesCours(_ sender: BtnPleinLarge) {
        ...
        if isReady {
            ...
            controller.semestre = self.semestre
        ...
        self.run(after: 4) {
            if self.isReady && !self.isMyCours {
                let controller = ListeMatieresViewController()
                controller.filiere = self.filiere
                controller.semestre = self.semestre
                self.navigationController?.pushViewController(controller, animated: true)
